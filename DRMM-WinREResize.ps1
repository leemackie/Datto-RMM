### WinReResize
### Adaption of the Microsoft script to resize the WinRE partition
### Version 1.0


$transcriptPath = "C:\ProgramData\CentraStage\Temp\DRMM-WinREResize-$($([DateTime]::Now).ToFileTime()).txt"
Write-Host "Starting script transcription to $transcriptPath"
Start-Transcript -Path $transcriptPath
[string]$dryRun=$ENV:DryRun
[string]$backupFolder=$ENV:Backup
[int]$UDF=$ENV:UDF
...
function Remove-Transcript($transcriptPath) {
	Stop-Transcript
	LogMessage("Deleting script transcription file as script ran successfully")
...
if (!$WinREStatus) {
	...
	New-ItemProperty -Path "HKLM:\SOFTWARE\CentraStage" -Name Custom$UDF -Value "WinRE Disabled" -Force | Out-Null
	exit 0
}
...
if ($WinREPartitionIndex -eq $OSPartition.PartitionNumber) {
	...
	Remove-Transcript($transcriptPath)
	exit 0
}
...
if ($WinREPartition.Offset -lt $OSPartitionEnds) {
	...
	if ($targetOSPartitionSize -lt $supportedSize.SizeMin) {
		LogMessage("Error: The target OS partition size after shrinking is smaller than the supported minimum size, cannot perform the repartition (#1)")
		LogMessage("Target OS Partition size: $targetOSPartitionSize")
		LogMessage("Acceptable OS partition size: $($supportedSize.SizeMin)")
		exit 1
	}
} else {
	if ($WinREIsOnSystemPartition) {
	...
	if ($UnallocatedSpace -ge 250MB) {
		...
		if ($targetOSPartitionSize -lt $supportedSize.SizeMin) {
			LogMessage("Error: The target OS partition size after shrinking is smaller than the supported minimum size, cannot perform the repartition (#2)")
			LogMessage("Target OS Partition size: $targetOSPartitionSize")
			LogMessage("Acceptable OS partition size: $($supportedSize.SizeMin)")
			exit 1
		}
...
if ($DryRun -eq "False") {
	LogMessage("User chose to skip dry run")
	LogMessage("Proceeding with changes...")
} else {
	LogMessage("Script is a dry run, canceling")
	Remove-Transcript($transcriptPath)
	exit 0
}
...
if ($diskType -ieq "GPT") {
	...
	$targetWinREPartitionSizeInMb = [int]($($targetWinREPartitionSize/1MB)-1)
	$diskpartScript =
@"
